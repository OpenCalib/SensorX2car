                                    ...
                                    bool applyWeight)
{
    ...
    if (used_frm_ % 2 == 0 ) {
        if (new_yaw > 0)
            printf("%dpts(origin)->%dpts(stationary): y = %.4fcos(x+%.3f)\n", ve.size(), pt_num, A, new_yaw*180/M_PI);
        else 
            printf("%dpts(origin)->%dpts(stationary): y = %.4fcos(x%.3f)\n", ve.size(), pt_num, A, new_yaw*180/M_PI);
    }
    ...
    std::string output_dir = output_dir_ + "/fitline/";
    ...
    if (used_frm_ % 10 == 0)
    {
        std::string txtfile = output_dir_ + "/fitline/" + std::to_string(pt_num) + "_" +
                                        ...
                                        const double &yaw)
{
    ...
    // }
    std::string output_dir = output_dir_ + "/stationary/";
    if (opendir(output_dir.c_str()) == nullptr)
    {
        char command[1024];
        snprintf(command, sizeof(command), "mkdir -p %s", output_dir.c_str());
    ...
    std::string output_file = output_dir + std::to_string(used_frm_) + "_" + 
